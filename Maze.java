 ...
 * @additional authors Saiyam Mahajan
 * @version 3.00	Update the application to JavaFX
 * @version 3.01    The library has been enhanced by means of putting 'X' 
 *                  symbol and shows the exact path from the "start" to the "goal" 
 *                  by using Iterative approach. Moreover, author's name has been added to
 *                  the console as well as the output.
 */
...
public class Maze extends Canvas {
	...
	private boolean makeMove(int r, int c) {
		...
		if(getCell(r,c)=='|') {        /* Usage of iterative approach (if-else statements) */
			return false;
		}	
		else if(getCell(r,c)=='+') {
			return false;
		...
		else if(getCell(r,c)=='G') {
			return false;
		}
		
		else if(getCell(r,c)=='X') {
			return false;
		}
		
		//Now, through this section we will make the cell empty to make the move
		...
		//Through the following section,  we will try to move the cell up.
		if (board[r - 1][c] == ' ') {
		nextRow = r - 1;
		nextCol = c;
		return true;
		}
		// Similarly, we will try to move the cell to the right.
		else if (board[r][c + 1] == ' ') {
		nextRow = r;
		nextCol = c + 1;
		return true;
